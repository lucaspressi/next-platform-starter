...
export default function ImageUpload({ item, setQuizItems }) {
      ...
      if (response.ok) {
        setQuizItems((prevItems) =>
          prevItems.map((quizItem) =>
            quizItem.id === item.id ? { ...quizItem, imagePreview: data.secure_url } : quizItem
          )
        );
      } else {
  ...
  const handleRemoveImage = () => {
    setQuizItems((prevItems) =>
      prevItems.map((quizItem) =>
        quizItem.id === item.id ? { ...quizItem, imagePreview: null } : quizItem
      )
    );
  };
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-start gap-2 text-gray-600 text-sm">
        ...
        <p>
          Opcional: Adicione uma imagem para tornar sua pergunta mais personalizada e divertida.
          Pode ser uma foto especial do casal ou uma imagem que represente a pergunta.
        </p>
      </div>
